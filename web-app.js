  ...
  <div class="ticket-card" data-id="${l.id}">
    ...
    <div class="main">
      ...
      <div class="card-actions">
        ...
        ` : `
          ${l.needsReceiptConfirmation ? `<button class="btn btn-primary btn-confirm-receipt" style="width:100%; margin-bottom:8px;">✅ ยืนยันว่าได้รับบัตรแล้ว (ผู้ขายจะได้รับเงิน)</button>` : ''}
          ${status !== 'sold' && state.paymentsEnabled ? `<button class="btn btn-primary btn-pay-qr" style="width:100%; margin-bottom:8px;">🛒 ซื้อเลย — จ่ายผ่าน PromptPay</button>` : ''}
          <button class="link-btn btn-reveal">ดูข้อมูลติดต่อ/บัญชีผู้ขาย</button>
          <button class="link-btn btn-chat-seller">💬 แชทกับผู้ขาย</button>
          ${status !== 'sold' ? `<button class="link-btn btn-notice">แจ้งโอนเงินแล้ว</button>` : ''}
        `}
      </div>
      ${mine ? `
        <div class="seller-controls">
          ${status === 'sold' ? `<div style="font-size:12px; color:var(--gold); margin-bottom:8px;">ขายแล้ว — เงินจะโอนให้คุณหลังผู้ซื้อกดยืนยันว่าได้รับบัตร (เช็คสถานะจ่ายเงินได้ที่หน้าแอดมิน)</div>` : ''}
          <button class="link-btn btn-chat-buyers" style="width:100%; margin-bottom:8px;">💬 ข้อความจากผู้ซื้อ</button>
...
function bindCardEvents(grid, list) {
  grid.querySelectorAll('.ticket-card').forEach(card => {
    ...
    const confirmReceiptBtn = card.querySelector('.btn-confirm-receipt');
    if (confirmReceiptBtn) confirmReceiptBtn.onclick = async () => {
      if (!confirm(`ยืนยันว่าคุณได้รับบัตร "${l.event}" แล้วใช่ไหม? การกดยืนยันจะทำให้ระบบโอนเงินให้ผู้ขายได้`)) return;
      try {
        await api(`/listings/${l.id}/confirm-received`, { method: 'POST' });
        await loadListings(); renderAll();
        toast('ยืนยันรับบัตรแล้ว ขอบคุณครับ', 'ok');
      } catch (e) { toast(e.message, 'err'); }
    };
    const chatSellerBtn = card.querySelector('.btn-chat-seller');
    if (chatSellerBtn) chatSellerBtn.onclick = () => openListingChatModal(l);
    const chatBuyersBtn = card.querySelector('.btn-chat-buyers');
    if (chatBuyersBtn) chatBuyersBtn.onclick = () => openListingBuyerThreadsModal(l);
...
// Buyer side: their own conversation with this listing's seller.
function openListingChatModal(l) {
  if (!state.token) { openAuthModal('login'); toast('กรุณาเข้าสู่ระบบก่อนแชทกับผู้ขาย'); return; }
  if (state.username === l.seller) { toast('นี่คือประกาศของคุณเอง'); return; }
  const root = document.getElementById('modalRoot');
  root.innerHTML = `
  <div class="modal-overlay" id="overlay">
    <div class="modal chat-modal">
      <button class="close" id="closeBtn">&times;</button>
      <h3>💬 แชทกับผู้ขาย</h3>
      <div class="modal-sub">"${escapeHtml(l.event)}" · ผู้ขาย: ${escapeHtml(l.seller)}</div>
      <div class="chat-thread" id="chatThread"></div>
      <div class="chat-input-row">
        <input id="chatInput" type="text" placeholder="พิมพ์ข้อความ...">
        <button class="btn btn-primary" id="chatSendBtn" style="width:auto; padding:11px 18px;">ส่ง</button>
  ...
  const refresh = async () => {
    ...
    if (!thread) return;
    try {
      const msgs = await api(`/listings/${l.id}/chat/mine`);
      thread.innerHTML = msgs.length
        ? msgs.map(m => chatBubbleHtml(m, m.sender === 'buyer')).join('')
        : `<div style="text-align:center; color:var(--muted); font-size:13px; padding:20px 0;">ยังไม่มีข้อความ พิมพ์ทักผู้ขายได้เลย</div>`;
      thread.scrollTop = thread.scrollHeight;
    } catch (e) { /* ignore */ }
  };
  refresh();
  const sendFn = async () => {
    ...
    input.value = '';
    try {
      await api(`/listings/${l.id}/chat/mine`, { method: 'POST', body: JSON.stringify({ text }) });
      await refresh();
    } catch (e) { toast(e.message, 'err'); }
  };
  document.getElementById('chatSendBtn').onclick = sendFn;
...
// Seller side: list every buyer thread for this listing, pick one to reply to.
async function openListingBuyerThreadsModal(l) {
  const root = document.getElementById('modalRoot');
  root.innerHTML = `
  <div class="modal-overlay" id="overlay">
    <div class="modal chat-modal" style="max-width:460px;">
      <button class="close" id="closeBtn">&times;</button>
      <h3>💬 ข้อความจากผู้ซื้อ</h3>
      <div class="modal-sub">"${escapeHtml(l.event)}"</div>
      <div id="threadListArea"></div>
    </div>
  </div>`;
  document.getElementById('closeBtn').onclick = closeModal;
  document.getElementById('overlay').onclick = (e) => { if (e.target.id === 'overlay') closeModal(); };
  ...
  let threads = [];
  try { threads = await api(`/listings/${l.id}/chat/threads`); } catch (e) { toast(e.message, 'err'); }
  let activeBuyer = threads[0] ? threads[0].buyer : null;

  const draw = () => {
    const area = document.getElementById('threadListArea');
    if (!threads.length) {
      area.innerHTML = `<div style="text-align:center; color:var(--muted); font-size:13px; padding:16px 0;">ยังไม่มีผู้ซื้อทักมา</div>`;
      return;
    }
    area.innerHTML = `
      <div style="display:flex; gap:6px; flex-wrap:wrap; margin-bottom:12px;">
        ${threads.map(t => `<button class="link-btn buyer-tab" data-buyer="${escapeHtml(t.buyer)}" style="${t.buyer === activeBuyer ? 'border-color:var(--gold); color:var(--gold);' : ''}">${escapeHtml(t.buyer)}</button>`).join('')}
      </div>
      <div class="chat-thread" id="chatThread"></div>
      <div class="chat-input-row">
        <input id="chatInput" type="text" placeholder="พิมพ์ตอบกลับ...">
        <button class="btn btn-primary" id="chatSendBtn" style="width:auto; padding:11px 18px;">ส่ง</button>
      </div>
    `;
    const active = threads.find(t => t.buyer === activeBuyer);
    document.getElementById('chatThread').innerHTML = active
      ? active.messages.map(m => chatBubbleHtml(m, m.sender === 'seller')).join('')
      : '';
    area.querySelectorAll('.buyer-tab').forEach(btn => {
      btn.onclick = () => { activeBuyer = btn.dataset.buyer; draw(); };
    });
    const sendFn = async () => {
      const input = document.getElementById('chatInput');
      const text = input.value.trim();
      ...
      input.value = '';
      try {
        await api(`/listings/${l.id}/chat/${encodeURIComponent(activeBuyer)}`, { method: 'POST', body: JSON.stringify({ text }) });
        threads = await api(`/listings/${l.id}/chat/threads`);
        draw();
      } catch (e) { toast(e.message, 'err'); }
    };
    document.getElementById('chatSendBtn').onclick = sendFn;
